...
const ItemsList = (props: {startingNumber:number, items:PointRecord[], recordLoaded(item:any), recordDeleted()}) => {

    return (
        <div style={{overflow:'scroll'}}>
        <table className="table table-striped">
            {tableHeader("No", "Student", "Date", "Point Name","Category", "Point",  "Description")}
            <tbody>
                    {props.items.map((item, i)=>{
                        ...
                        return (
                            <tr key={"PointRecord-"+i}>
                                ...
                                <td>{item.rule_point?.name}</td>
                                <td>{item.rule_point?.category?.name}</td>
